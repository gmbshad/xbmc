...
static int py_messaging_rec_set_prev(PyObject *py_obj, PyObject *value, void *closure)
{
	...
	if (value == NULL) {
		PyErr_Format(PyExc_AttributeError, "Cannot delete NDR object: struct object->prev");
		return -1;
	}
	if (value == Py_None) {
		object->prev = NULL;
...
static PyObject *py_messaging_rec_ndr_print(PyObject *py_obj, PyObject *Py_UNUSED(ignored))
{
	struct messaging_rec *object = (struct messaging_rec *)pytalloc_get_ptr(py_obj);
	PyObject *ret;
	char *retstr;
...
static PyObject *py_messaging_reclog_ndr_print(PyObject *py_obj, PyObject *Py_UNUSED(ignored))
{
	struct messaging_reclog *object = (struct messaging_reclog *)pytalloc_get_ptr(py_obj);
	PyObject *ret;
	char *retstr;
...
MODULE_INIT_FUNC(messaging)
{
	...
	PyModule_AddObject(m, "MSG_SMB_STAT_CACHE_DELETE", PyLong_FromUnsignedLongLong((uint32_t)MSG_SMB_STAT_CACHE_DELETE));
