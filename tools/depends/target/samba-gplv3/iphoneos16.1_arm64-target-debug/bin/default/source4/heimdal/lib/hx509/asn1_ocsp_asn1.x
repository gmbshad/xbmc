/* Generated from /Users/redd/kodi/tools/depends/target/samba-gplv3/iphoneos16.1_arm64-target-debug/source4/heimdal/lib/hx509/ocsp.asn1 */
/* Do not edit */
...
{
size_t requestList_tag_oldret = ret;
unsigned int n_requestList_tag;
ret = 0;
for(n_requestList_tag = (&(data)->requestList)->len; n_requestList_tag > 0; --n_requestList_tag){
size_t requestList_tag_for_oldret = ret;
ret = 0;
ret += length_OCSPInnerRequest(&(&(data)->requestList)->val[n_requestList_tag - 1]);
ret += requestList_tag_for_oldret;
}
ret += requestList_tag_oldret;
}
...
{
size_t certs_tag_tag_oldret = ret;
unsigned int n_certs_tag_tag;
ret = 0;
for(n_certs_tag_tag = ((data)->certs)->len; n_certs_tag_tag > 0; --n_certs_tag_tag){
size_t certs_tag_tag_for_oldret = ret;
ret = 0;
ret += length_Certificate(&((data)->certs)->val[n_certs_tag_tag - 1]);
ret += certs_tag_tag_for_oldret;
}
ret += certs_tag_tag_oldret;
}
...
{
size_t responses_tag_oldret = ret;
unsigned int n_responses_tag;
ret = 0;
for(n_responses_tag = (&(data)->responses)->len; n_responses_tag > 0; --n_responses_tag){
size_t responses_tag_for_oldret = ret;
ret = 0;
ret += length_OCSPSingleResponse(&(&(data)->responses)->val[n_responses_tag - 1]);
ret += responses_tag_for_oldret;
}
ret += responses_tag_oldret;
}
