#include "config.h"
/* Generated from /Users/redd/kodi/tools/depends/target/samba-gplv3/iphoneos16.1_arm64-target-debug/source4/heimdal/lib/asn1/pkcs12.asn1 */
/* Do not edit */
...
size_t Top_tag_for_oldret = ret;
ret = 0;
ret += length_ContentInfo(&(data)->val[n_Top_tag - 1]);
ret += Top_tag_for_oldret;
}
ret += Top_tag_oldret;
}
...
size_t Top_tag_for_oldret = ret;
ret = 0;
ret += length_PKCS12_Attribute(&(data)->val[n_Top_tag - 1]);
ret += Top_tag_for_oldret;
}
ret += Top_tag_oldret;
}
...
size_t Top_tag_for_oldret = ret;
ret = 0;
ret += length_PKCS12_SafeBag(&(data)->val[n_Top_tag - 1]);
ret += Top_tag_for_oldret;
}
ret += Top_tag_oldret;
}
